...
export const getAvailableChatLists = (companyId) => (dispatch) => {
    dispatch({ type: actionTypes._START_CHAT_LIST_LOADING });
    ...
    database
        ...
        .then((snapShot) => {
            ...
            dispatch({
                type: actionTypes._GET_STUDENT_METADATA,
        ...
        .then(() => {
            database
                ...
                .then((doc) => {
                    dispatch({
                        type: actionTypes._GET_COMPANY_METADATA,
                        payload: doc.data(),
                    });
                    dispatch({
                        type: actionTypes.GET_COMPANY_DETAILS, /* Revisit@rajsknvr **/
                        payload: doc.data(),
                    });
                    ...
                    database
                        ...
                        .onSnapshot((querySnapshot) => {
                            ...
                            dispatch({
                                type: actionTypes._GET_AVAILABLE_CHAT_LISTS,
                                payload: {
                            ...
                            dispatch({ type: actionTypes._STOP_CHAT_LIST_LOADING });
                        });
                });
        });
};
...
export const setSelectedChat = (key) => (dispatch) => {
    ...
    dispatch({
        type: actionTypes._SET_SELECTED_CHAT,
...
export const setChatReadStatus = (chatId) => (dispatch) => {
    ...
    database.collection("ChatRoomGalkLab").doc(chatId).get().then(doc => {
        let data = doc.data();
...
export const suspendChat = () => (dispatch) => {
    const chatId = store.getState().chatRoomGalkLab.selectedChatId;
...
export const submitChatMsg = (msg, type) => (dispatch) => {
    ...
    if (type === "image") {
        dispatch({
            type: actionTypes._SET_CHAT_FILEUPLOAD_LOADING,
            payload: true,
        });
        ...
        const uploadTask = storageRef
            .child(
                `ChatRoomGalkLab/Image/${chatId}/${senderId}/${uuid()}/${fileToUpload.name}`
            )
        ...
        uploadTask.on(
            ...
            (snapshot) => {
                ...
                dispatch({
                    type: actionTypes._SET_CHAT_FILEUPLOAD_PROGRESS,
                    payload: progress,
                });
            },
            (err) => {
    ...
    if (type === "video") {
        ...
        const uploadTask = storageRef
            .child(
                `ChatRoomGalkLab/Video/${chatId}/${senderId}/${uuid()}/${fileToUpload.name}`
            )
            .put(fileToUpload, { contentType: fileToUpload.type });
    ...
    if (type === "document") {
        ...
        const uploadTask = storageRef
            .child(
                `ChatRoomGalkLab/Document/${chatId}/${senderId}/${uuid()}/${fileToUpload.name}`
            )
            .put(fileToUpload, { contentType: fileToUpload.type });
...
export const clearChatRoomData = () => (dispatch) => {
    dispatch({ type: actionTypes._CLEAR_CHAT_ROOM_DATA });
};
...
export const removeGroupChatMember = (id) => (dispatch) => {
    ...
    const studentToRemoveName = store
        .getState()
        .chatRoomGalkLab.studentMetaData.find((s) => s.id === id).name;
...
export const addGroupChatMember = (idList) => (dispatch) => {
    ...
    const studentMetaData = store.getState().chatRoomGalkLab.studentMetaData;
