...
import { Card, Row, Col, Button, Modal, Empty, Divider } from "antd";
import { 
...
const SearchResult = ({
	...
	const statusDistribution = {"Third year student": 0, "Fourth year student": 0, "Graduate": 0};
	const branchDistribution = {};
	const campusDistribution = {};
	const fieldsName = [
		"Skills", "Status", "Branch", "Campus"
	];
	...
	const getTopNFields = (field, n, fieldName) => {
		...
		if (fieldName == "Branch" || fieldName == "Campus" || fieldName == "Status")
	...
	const updateStats = (student, isLast) => {
		...
		const admissionYear = student.yearOfAdmission
		...
		if (admissionYear){
			if (admissionYear  == admissionYearForThirdYearStudents){
				statusDistribution["Third year student"] += 1
			} else if (admissionYear  == admissionYearForFourthYearStudents){
				statusDistribution["Fourth year student"] += 1
			} else {
				statusDistribution["Graduate"] += 1
			} 
		}
		...
		if (isLast){
			getTopNFields(skillsDistribution, 20, fieldsName[0])
			getTopNFields(statusDistribution, 3, fieldsName[1])
			getTopNFields(branchDistribution, 6, fieldsName[2])
			getTopNFields(campusDistribution, 15, fieldsName[3])
		}
	}
		...
		<>
			...
			>
				<Row gutter={8} style={{ margin: "auto" }}>
					...
					>
						{t("total_engineers_registered")} {studentList.length}
					</div>
					<Divider style={{'background-color':'black'}}/>

					<div style={{"display": "flex", "margin": "auto"}}>
						<canvas id={fieldsName[0]} width="1100px" height="450px"></canvas>
					</div>
					<Divider style={{'background-color':'black'}}/>
					...
					<Col style={{ margin: "auto", display: "flex"  }}>
						...
						<Row>
							<canvas id={fieldsName[2]} width="550px" height="450px"></canvas>
						</Row>
					</Col>
					<Divider style={{'background-color':'black'}}/>

					<Col style={{ margin: "auto", display: "flex"  }}>
						<canvas id={fieldsName[3]} width="850px" height="450px"></canvas>
					</Col>
					<Divider style={{'background-color':'black'}}/>
				</Row>
			</Card>
