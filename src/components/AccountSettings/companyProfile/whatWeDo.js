...
const WhatWeDo = ({
	...
	const updateInformation = (values) => {
		...
		if (values.do == "") setCompletionBool("whatWeDo", 0)
		else setCompletionBool("whatWeDo")
	};
	return (
		<Card
			type="inner"
			title={`${t("accountsetting_companyprofile_do_title")}`}
			extra={
				isEditable ? (
					<>
						<Button
		...
		>
			{Object.entries(companyBasicDetails).length > 0 && (
				<Form
					...
					initialValues={
						companyBasicDetails && {
							do: companyBasicDetails.do,
						}
					}
					onFinish={updateInformation}
				>
					<Form.Item
						// label={`${t("accountsetting_companyprofile_do_briefdesc")}`}
						name="do"
						// rules={[{ required: true, message: "Please enter information!" }]}
					>
						<Input.TextArea
							placeholder="Brief about what your company does.."
							disabled={!isEditable}
