...
const Logo = ({ companyLogo, updateCompanyLogo, isActionProgress }) => {
	...
	const updateLogo = () => {
		if (logoObject) {
			...
			setCompletionBool("corporateLogo")
		} else {
			message.error("Please select a new file and then click on save.");
		}
	};
		...
		>
			<div className="companyDetails_logo_container">
				...
				<div className="companyDetails_logo_rule">
					...
					<Badge status="warning" text={`${t("imageupload_instruction3")}`} />
				</div>
				<div>
					<ImgCrop rotate>
						<Dragger
...
const mapStateToProps = (state) => ({
	companyLogo: state.company.company ? state.company.company.logo : null,
