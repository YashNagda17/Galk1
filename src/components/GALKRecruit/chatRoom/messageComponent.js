...
const renderMsgBasedOnType = (msg, isRestrictedAccountType) => {
	if (msg.msgType) {
		switch (msg.msgType) {
			case "image":
				return (
					...
					<span
						className={
							isRestrictedAccountType
								? "chatRoom_chatRoom-systemMsg-msg blur"
								: "chatRoom_chatRoom-systemMsg-msg"
						}
					>
						{msg.message}
					</span>
			...
			{messageList ? (
				messageList.map((msg, i) => (
					<div key={i} className={getSentMsgClassName(msg.sender, companyId)}>
						...
						{renderMsgBasedOnType(msg, isRestrictedAccountType)}
					</div>
				))
			) : (
